  ...
  return (
    <div className="w-full min-h-dvh flex flex-col items-center justify-center p-6">
      ...
      <div aria-hidden className="pointer-events-none fixed inset-0">
        <div
          className="absolute top-[-20%] left-[-15%] w-[60vw] h-[60vw] rounded-full opacity-[0.07]"
          style={{ background: `radial-gradient(circle,${glowColor} 0%,transparent 70%)`, filter: "blur(80px)" }}
        />
        <div
          className="absolute bottom-[-20%] right-[-15%] w-[60vw] h-[60vw] rounded-full opacity-[0.07]"
          style={{ background: "radial-gradient(circle,var(--sheriff-gold) 0%,transparent 70%)", filter: "blur(80px)" }}
        />
      </div>

      <div className="relative z-10 w-full max-w-2xl flex flex-col gap-8 mx-auto">
        ...
        >
          ...
          <p className="text-text-muted text-xs uppercase tracking-widest mt-1">{subtitle}</p>
        </motion.div>
...
function CategoryPicker({ onSelect, onBack }: { onSelect: (id: string) => void; onBack: () => void }) {
  return (
    <PickerShell
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {QUESTION_CATEGORIES.map((cat, i) => (
          <motion.button
            ...
            className="flex flex-col gap-2 p-5 rounded-2xl border-2 text-left transition-all duration-200"
          ...
          >
            ...
            <p className="text-xs font-semibold mt-1" style={{ color: cat.color, opacity: 0.6 }}>
...
function NeverSourcePicker({ onSelect, onBack }: { onSelect: (src: NeverSource) => void; onBack: () => void }) {
  return (
    <PickerShell
      ...
      <div className="flex flex-col gap-4">
        {NEVER_SOURCES.map((src, i) => (
          <motion.button
            ...
            className="flex flex-col gap-1.5 p-5 rounded-2xl border-2 text-left transition-all duration-200"
          ...
          >
            <p className="font-bold text-base" style={{ color: src.color }}>{src.label}</p>
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            className="w-full bg-saloon-surface border-2 rounded-xl px-4 py-3 text-base font-bold text-text-primary placeholder:text-text-muted focus:outline-none transition-colors"
            style={{
        ...
        >
          {loading ? <Loader2 size={18} className="animate-spin" /> : "Utwórz drużyny i otwórz lobby"}
        </motion.button>
      </motion.div>
    </PickerShell>
  );
...
export default function HostPage() {
  ...
  const appNeverCards = useMemo(
    () => fullDeck.filter((c) => c.type === "NEVER"),
    [fullDeck],
  );
  const appQuizCards = useMemo(
    () => fullDeck.filter((c) => c.type === "QUIZ"),
    [fullDeck],
  );
