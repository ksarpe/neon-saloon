...
const DATABASE_ID =
  process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID ?? "lastrodeo";
...
function dispatch(
  ...
  switch (type) {
    case "player-joined":
      handlers.onPlayerJoined?.(data as Parameters<NonNullable<GameSocketHandlers["onPlayerJoined"]>>[0]);
      break;
    case "player-left":
      handlers.onPlayerLeft?.(data as Parameters<NonNullable<GameSocketHandlers["onPlayerLeft"]>>[0]);
      break;
    case "team-created":
      handlers.onTeamCreated?.(data as Parameters<NonNullable<GameSocketHandlers["onTeamCreated"]>>[0]);
      break;
    case "team-updated":
      handlers.onTeamUpdated?.(data as Parameters<NonNullable<GameSocketHandlers["onTeamUpdated"]>>[0]);
      break;
    case "vote-cast":
      handlers.onVoteCast?.(data as Parameters<NonNullable<GameSocketHandlers["onVoteCast"]>>[0]);
      break;
    case "votes-revealed":
      handlers.onVotesRevealed?.(data as Parameters<NonNullable<GameSocketHandlers["onVotesRevealed"]>>[0]);
      break;
    case "next-card":
      handlers.onNextCard?.(data as Parameters<NonNullable<GameSocketHandlers["onNextCard"]>>[0]);
      break;
    case "game-started":
      handlers.onGameStarted?.(data as Parameters<NonNullable<GameSocketHandlers["onGameStarted"]>>[0]);
      break;
    case "game-finished":
      handlers.onGameFinished?.(data as Parameters<NonNullable<GameSocketHandlers["onGameFinished"]>>[0]);
      break;
    case "highlow-round-start":
      handlers.onHighLowRoundStart?.(data as Parameters<NonNullable<GameSocketHandlers["onHighLowRoundStart"]>>[0]);
      break;
    case "highlow-number-submitted":
      handlers.onHighLowNumberSubmitted?.(data as Parameters<NonNullable<GameSocketHandlers["onHighLowNumberSubmitted"]>>[0]);
      break;
...
function maxSeq(events: StoredEvent[]): number {
  return events.length === 0
    ? -1
    : events.reduce((m, e) => (e.seq > m ? e.seq : m), -1);
}
...
export function useGameEvents(
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      console.log(`[Appwrite RT] subscribed to row pin=${pin} (baseline seq=${lastSeenSeq})`);
    })();
